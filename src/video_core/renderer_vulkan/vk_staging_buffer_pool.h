...
public:
    ...
    /// Callable from any thread (readbacks complete on the scheduler's priority thread); the
    /// release is applied by the owning thread on its next request or frame tick.
...
private:
    ...
    std::vector<const VideoCore::Buffer*> releases;
    std::vector<const VideoCore::Buffer*> releasing;
};
