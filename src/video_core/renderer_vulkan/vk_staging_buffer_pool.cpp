...
void StagingBufferPool::FreeDeferred(const StagingBufferRef& ref) {
    ...
    releases.push_back(ref.buffer);
}

void StagingBufferPool::ApplyReleases() {
    {
        std::scoped_lock lock{release_mutex};
        if (releases.empty()) {
            return;
        }
        releasing.swap(releases);
    }
    for (const VideoCore::Buffer* buffer : releasing) {
        const auto held = [&](const LargeBuffer& entry) { return entry.buffer.get() == buffer; };
        bool found = false;
        for (auto& cache : large_caches) {
            if (const auto it = std::ranges::find_if(cache, held); it != cache.end()) {
                ASSERT_MSG(it->held, "FreeDeferred on an allocation that is not held");
                it->held = false;
        ...
        ASSERT_MSG(found, "FreeDeferred on an unknown allocation");
    }
    releasing.clear();
}
