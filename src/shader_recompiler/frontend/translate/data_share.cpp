    ...
    if (offset1 & 0x80) {
        if (offset0 == 0x0 || offset0 == 0x55 || offset0 == 0xAA || offset0 == 0xFF) {
            ...
            if (info.hw_stage == HwStage::Fragment) {
                // Helper invocations are only guaranteed to take part in quad operations.
                // The backend expands a lane-dependent QuadBroadcast index into four
                // constant quad broadcasts plus a per-lane select.
                SetDst(inst.dst[0], ir.QuadBroadcast(src, sel));
            } else {
