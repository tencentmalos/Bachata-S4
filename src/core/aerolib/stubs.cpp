...
// Stubs are host code called directly by the guest, so they only exist on x86-64 hosts; the
// native ARM64 host resolves missing imports through the guest runtime instead.
...
static u64 PS4_SYSV_ABI CommonStub(u64 index) {
    // Modules loaded later can add stubs (and grow the table) while guest threads run.
...
#else
u64 GetStub(const char* nid) {
    throw std::runtime_error(std::string("host stub requested on a non-x86-64 host for nid ") +
                             nid);
}
#endif
