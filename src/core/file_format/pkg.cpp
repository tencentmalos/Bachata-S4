...
bool PKG::Extract(const std::filesystem::path& filepath, const std::filesystem::path& extract,
    ...
    for (int i = 0; i < n_files; i++) {
        ...
        if (entry.id == 0x1) {         // DIGESTS, seek;
            ...
            Core::Crypto::RSA2048Decrypt(dk3_, key1[3], true); // decrypt DK3
            ...
            Core::Crypto::IvKeyHash256(concatenated_ivkey_dk3, ivKey); // ivkey_
            // imgkey_ to use for last step to get ekpfs
            Core::Crypto::AesCbcCfb128Decrypt(ivKey, imgkeydata, imgKey);
            // ekpfs key to get data and tweak keys.
            Core::Crypto::RSA2048Decrypt(ekpfsKey, imgKey, false);
        ...
        if (entry.id == 0x400 || entry.id == 0x401 || entry.id == 0x402 ||
            ...
            // These entries are not block-aligned. Decrypt a zero-padded copy and
            // write back only entry.size bytes, so the trailing partial block is
            // decrypted against zeros rather than left as ciphertext.
            const u32 padded_size = Common::AlignUp<u32>(entry.size, 16);
            ...
            std::span<u8> cipherNp(data.data(), padded_size);
            ...
            Core::Crypto::IvKeyHash256(concatenated_ivkey_dk3_, ivKey);
            Core::Crypto::AesCbcCfb128Decrypt(ivKey, cipherNp, decNp);
    ...
    Core::Crypto::PfsGenCryptoKey(ekpfsKey, seed, dataKey, tweakKey);
    ...
    if (length != 0) {
        ...
        Core::Crypto::DecryptPFS(dataKey, tweakKey, pfs_encrypted, pfs_decrypted, 0);
        ...
        if (pfsc_offset == static_cast<u32>(-1) || pfsc_offset >= length) {
            // Key derivation produced garbage: the package is retail-signed rather
            // than fake-signed, so the bundled keys cannot unwrap it.
            failreason = "PFSC magic not found; PKG is not fake-signed";
            return false;
        }
...
void PKG::ExtractFiles(const int index) {
    ...
    if (inode_type == PFS_FILE) {
        ...
        for (int j = 0; j < nblocks; j++) {
            ...
            Core::Crypto::DecryptPFS(dataKey, tweakKey, pfsc, pfs_decrypted, currentSector1);
