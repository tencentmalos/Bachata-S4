...
class AudioRuntimeInstrumentedTest {
    @Test fun oboeConsumesGuestBuffersAndClosesAcrossRestarts() {
        ...
        val root = File(context.filesDir, "validation/audio-${System.nanoTime()}").apply { mkdirs() }
        try {
            listOf("audio", "audio", "audio").forEachIndexed { round, name ->
                ...
                val gen = NativeFexSession.nativeStartExecutable("audio", entry.path)
                assertTrue(gen > 0)
                try {
                    val outcome = NativeFexSession.nativeWaitTerminal(gen, 15000)
                    val detail = NativeFexSession.nativeTerminalDetail(gen).orEmpty()
                    android.util.Log.i("AudioAcceptance", "round=$round case=$name outcome=$outcome ${NativeFexSession.nativeIdentity()} $detail")
