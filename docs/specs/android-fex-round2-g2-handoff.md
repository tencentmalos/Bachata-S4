...
| M02 | 持 token unmap/remap 同 VA，换 backing 和代码后执行 | 100 次；旧字节/旧翻译不能命中，mapping 与 code generation 都有证据 |
| M03 | 真实 ClearCodeCache 与失败注入；多线程 cache 一起处理 | 按 spec 次数；运行态调用拒绝、错误/过期 token 拒绝、失败后不能恢复旧译码 |
