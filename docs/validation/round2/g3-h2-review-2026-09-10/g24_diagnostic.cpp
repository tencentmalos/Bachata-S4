...
void TestCoordinatorRecovery(Harness& h) {
    ...
    for (int iteration = 0; iteration < 100; ++iteration) {
        ...
        (void)Await(resumed);
        if (!(closed && recovered && finished)) {
            printf("G24_DIAG iteration=%d ordering=%d external=%d closed=%d recovered=%d finished=%d late=%d other=%d receipt=%d cancelled=%d reason=%s progress_before=%llu after=%llu\n",iteration,ordering,int(external_reason),closed,recovered,bool(finished),bool(stopped),bool(other_stopped),bool(receipt),bool(cancelled),cancelled?std::string(ToString(cancelled.Value().primary_reason)).c_str():"error",(unsigned long long)before,(unsigned long long)Progress(progress));
        }
