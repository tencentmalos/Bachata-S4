...
files=[root/'build/wp1-review/device/guest_thread_attribute_tests',pathlib.Path('/Users/bytedance/Library/Android/sdk/ndk/29.0.14206865/toolchains/llvm/prebuilt/darwin-x86_64/sysroot/usr/lib/aarch64-linux-android/libc++_shared.so')]
m={'scope':'only thread attribute domain; no full regression','source':run(['git','rev-parse','HEAD']).strip(),'sources':{},'remote':remote,'files':{}}
for p in ['src/core/host_runtime/guest_thread_attributes.h','tests/host_runtime/guest_thread_attribute_tests.cpp']:m['sources'][p]=hashlib.sha256((root/p).read_bytes()).hexdigest()
try:
 ...
 run(adb+['shell','chmod','700',remote+'/guest_thread_attribute_tests'])
 result=subprocess.run(adb+['shell',f'LD_LIBRARY_PATH={remote} timeout -s KILL 30 {remote}/guest_thread_attribute_tests'],capture_output=True,text=True,timeout=35)
