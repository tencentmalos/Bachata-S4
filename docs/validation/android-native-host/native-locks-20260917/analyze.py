...
class Reduced:
 def __init__(self,start,end,check,progress):
  self.stacks=collections.defaultdict(list);self.rows={};self.events=0;self.bookmarks=[];self.progress={};self.bad=0;self.threads={}
 ...
 def append(self,e):
  ...
  if e.kind=='SpanBegin':self.stacks[e.tid].append((e.name,e.ts_ns))
   ...
   if not name.startswith(('HLE.scePthreadMutex','HLE.pthread_mutex','HLE.scePthreadCond','HLE.pthread_cond','HLE.sceKernelUsleep','HLE.sceKernelPread','HLE.sceKernelRead','GNM.SubmissionGate')):return
 ...
 def finish(self,kind,label):
  ...
  return dict(events=self.events,unmatched=self.bad,open_scopes=sum(map(len,self.stacks.values())),main_tid=main,threads=self.threads,first_present_ms=(present[0]['t']-starts[0]['t'])/1e6 if len(starts)==len(present)==1 else None,bookmarks=self.bookmarks,main=[g for g in self.rows.values() if g['tid']==main],rows=list(self.rows.values()),progress=list(self.progress.values()),diagnostics=[])
