#!/usr/bin/env bash
# One A/B phase for GPU write-watch mprotect coalescing: bash ab_watch.sh <outdir> <label> <on|off>
set -u
...
d upload_diag watch_coalesce $MODE > "$OUT/$L-mode.txt"
...
echo "$L watch_coalesce=$MODE fps=$(python -c "print(round(($b-$a)*1e9/($t1-$t0),3))") draws/frame=$(per attachment_draws) watch_calls=$(per watch_calls) watch_pages=$(per watch_pages) release_calls=$(per release_calls) syscalls=$(per syscalls) cpuset=$(adb -s $S shell 'cat /proc/$(pidof com.shadps4.android)/cpuset' | tr -d '\r')" | tee -a "$OUT/ab-fps.txt"
