#!/system/bin/sh
# Bounded sched/KGSL ftrace capture in an isolated tracefs instance (root).
# Usage: sh sched-capture.sh <pid> <seconds> <outdir>
set -eu
PID="$1"; SECS="$2"; D="$3"
I=/sys/kernel/tracing/instances/sync_review_$$
mkdir -p "$D"
...
for E in sched/sched_switch sched/sched_waking sched/sched_wakeup kgsl/adreno_cmdbatch_submitted kgsl/adreno_cmdbatch_retired kgsl/kgsl_waittimestamp_entry kgsl/kgsl_waittimestamp_exit kgsl/kgsl_pwrstats kgsl/kgsl_gpubusy kgsl/gpu_frequency power/cpu_frequency; do
