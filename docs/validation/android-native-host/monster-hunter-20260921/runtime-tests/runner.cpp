  ...
  try {
    ...
    for (unsigned round = 1; round <= 3; ++round) {
      ...
      bool pass =
          ...
          : mode == "fault"
              ? (report.outcome == RunOutcome::Faulted || report.outcome == RunOutcome::BackendFailed) &&
                    report.detail.find(argc == 5 ? argv[4] : "operation=") != std::string::npos
              : report.outcome == RunOutcome::Returned &&
                    report.detail.starts_with(std::string("guest return=") + (argc == 5 ? argv[4] : "51966") + "\n");
      if (!requested)
        requested = backend.RequestCancel(*runtime);
      if (!*requested)
        pass = false;
