// Review-only Android ARM64 probe; links the production COMMON implementation.
...
static bool RealFault(bool write) {
    ...
    if (pid == 0) {
        ...
        if (sigaction(SIGSEGV, &action, nullptr) != 0) _exit(82);
        const size_t page_size = sysconf(_SC_PAGESIZE);
        void* p = mmap(nullptr, page_size, PROT_READ | PROT_WRITE,
                       MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        if (p == MAP_FAILED || mprotect(p, page_size, write ? PROT_READ : PROT_NONE) != 0) _exit(83);
        if (write) *static_cast<volatile unsigned char*>(p) = 42;
        else { const auto byte = *static_cast<volatile unsigned char*>(p); (void)byte; }
...
int main() {
    ...
    first->magic = ESR_MAGIC; first->size = sizeof(esr_context);
    ...
    first->magic = 0x1234; first->size = 16;
    esr = reinterpret_cast<esr_context*>(ctx.uc_mcontext.__reserved+16);
    esr->head.magic = ESR_MAGIC; esr->head.size = sizeof(esr_context); esr->esr = 0x90000040;
    ...
    std::memset(&ctx,0,sizeof(ctx));
    first = reinterpret_cast<_aarch64_ctx*>(ctx.uc_mcontext.__reserved);
    first->magic=0x1234; first->size=12;
    esr_context bad{};bad.head.magic=ESR_MAGIC;bad.head.size=sizeof(bad);bad.esr=0x90000040;
    std::memcpy(ctx.uc_mcontext.__reserved+12,&bad,sizeof(bad));
    const bool accepts_misaligned = Common::IsWriteError(&ctx);
    std::printf("OBSERVATION misaligned_record_accepted=%d\n", accepts_misaligned);
