...
out=Path(__file__).resolve().parent;adb=['adb','-s','9c2841a4'];service=['shell','dumpsys','activity','service','com.shadps4.android/.service.FexSessionService']
...
assert pid==32104 and a['identity'][1]==2
(out/'scene-before.png').write_bytes(call(['exec-out','screencap','-p','-d','4630946441858561667']))
log=(out/'simpleperf-record.log').open('w')
proc=subprocess.Popen(adb+['shell','run-as','com.shadps4.android','/system/bin/simpleperf','record','-p',str(pid),'-e','cpu-clock:u','-f','100','--call-graph','fp','--duration','45','-o','files/rooftop-20260916-perf.data'],stdout=log,stderr=subprocess.STDOUT)
try:
...
(out/'perf.data').write_bytes(call(['exec-out','run-as','com.shadps4.android','cat','files/rooftop-20260916-perf.data']))
