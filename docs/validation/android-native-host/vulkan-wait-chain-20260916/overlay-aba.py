...
try:
 for i,mode in enumerate(['show','hide','show']):
  ...
  before,raw=state();assert before['identity'][:2]==['8463','2'];(p/f'overlay-{i}-before.txt').write_text(raw)
  (p/f'overlay-{i}-before.png').write_bytes(call(['exec-out','screencap','-p','-d','4630946441858561667']))
  time.sleep(20)
