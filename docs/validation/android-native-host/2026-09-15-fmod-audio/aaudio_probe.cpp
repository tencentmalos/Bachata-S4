...
aaudio_data_callback_result_t callback(AAudioStream*, void* user, void* data, int32_t n) {
    ...
    std::memset(data, 0, size_t(n) * s.channels * s.bytes);
...
int main() {
    ...
    for (int mode=9; mode<13; ++mode) {
        const int channels = (mode==0 || mode==2 || mode==6) ? 8 : 2;
        const bool cb=mode>=2, tune=mode==4 || mode==7 || mode==8;
        CallbackState state; state.channels=channels; state.bytes=(mode==10 || mode==11) ? 2 : 4;
        AAudioStreamBuilder* b{}; AAudioStream* stream{};
        auto result = AAudio_createStreamBuilder(&b);
        if (result) return 2;
        AAudioStreamBuilder_setDirection(b, AAUDIO_DIRECTION_OUTPUT);
        AAudioStreamBuilder_setSharingMode(b, mode==8 ? AAUDIO_SHARING_MODE_EXCLUSIVE : AAUDIO_SHARING_MODE_SHARED);
        AAudioStreamBuilder_setPerformanceMode(b, AAUDIO_PERFORMANCE_MODE_LOW_LATENCY);
        AAudioStreamBuilder_setFormat(b, state.bytes==2 ? AAUDIO_FORMAT_PCM_I16 : AAUDIO_FORMAT_PCM_FLOAT);
        AAudioStreamBuilder_setSampleRate(b,48000);
        AAudioStreamBuilder_setChannelCount(b,channels);
        if (mode==9 || mode==11 || mode==12) AAudioStreamBuilder_setChannelMask(b,AAUDIO_CHANNEL_STEREO);
        if (mode==11 || mode==12) AAudioStreamBuilder_setBufferCapacityInFrames(b,384);
        AAudioStreamBuilder_setUsage(b,AAUDIO_USAGE_GAME);
        if (mode<5) AAudioStreamBuilder_setBufferCapacityInFrames(b,2048);
        ...
        if (AAudioStream_getChannelCount(stream)!=channels || AAudioStream_getFormat(stream)!=(state.bytes==2 ? AAUDIO_FORMAT_PCM_I16 : AAUDIO_FORMAT_PCM_FLOAT)) {
