...
out=[]
try:
 for i,mode in enumerate(['start','stop','start']):
  print(call(d+['gpu_timing',mode]).decode(),flush=True);time.sleep(3)
  before,raw=state();assert before['identity'][:2]==['10213','1'];(p/f'timing-{i}-before.txt').write_text(raw)
  time.sleep(15)
  after,raw=state();assert before['identity']==after['identity'];(p/f'timing-{i}-after.txt').write_text(raw)
  ...
  (p/'timing-aba.json').write_text(json.dumps(out,indent=2))
finally: print(call(d+['gpu_timing','start']).decode(),flush=True)
