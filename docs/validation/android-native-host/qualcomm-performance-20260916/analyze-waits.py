...
wanted={10284,10320,10322,10324,10326,10358}
...
for line in (p/'sched.txt').read_text().splitlines():
...
j=json.loads((p/'handoff.json').read_text());opened={};spans=[]
...
def merge(seq):
 ...
 return out
out={'trace_bounds_ns':[min(ts_all),max(ts_all)],'trace_header':(p/'sched.txt').read_text()[:160], 'blocked_reasons':collections.Counter(r['event'] for r in blocked),'threads':[],'scope_scheduling':[]}
