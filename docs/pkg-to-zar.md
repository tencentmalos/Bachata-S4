...
```
files/games/CUSA50828/
  CUSA50828.zar
  CUSA50828-UPD.zar
  sce_sys/param.sfo       bounded library metadata cache
  sce_sys/icon0.png       optional library icon cache
  install.manifest       generated by library discovery
```

The library validates archive metadata and the effective update TITLE_ID,
then generates the small metadata cache and manifest. It launches the base
archive directly; ELF/SPRX loading, file reads and read-only mmap use the
layered archive namespace. A deployed archive must finish copying and pass
size/hash verification before it is renamed from `.partial` to `.zar`.
The Android SAF import UI still targets PKG; this deployment uses app-private
files and library discovery. See [Swan evidence](validation/android-native-host/swan-zar-20260920.md)
for actual validation and remaining boundaries.
