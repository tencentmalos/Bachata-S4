...
- Read [the transaction hardening record](docs/validation/v0/transaction-hardening-2026-09-08.md) before implementation, then [the prior poison follow-up](docs/validation/v0/followup-poison-2026-09-08.md). The main adapter now retains the CreateThread execution lease for the whole call; failed ranges are tracked independently; pinned-write invalidation failures and sink exceptions block execution; drain excludes replacement registration and mapping mutations cannot enter publication/execution. Swan contract 34/34 and real guest 45/45 pass. The stricter runner reports 11 complete acceptance PASS / 0 FAIL / 46 NOT_RUN of 57 in-scope cases, with B04/M02/M03 deferred. Partial unit evidence is auxiliary, not full acceptance. Build entry remains cmake/fex with guest_cpu_fex; scripts/android/build-fexcore-android now delegates its embedder build to that entry. Still V0_IN_PROGRESS: running-thread interruption, real HLE/callback, app/JNI/Surface, bounded Step and debug integration remain incomplete.
