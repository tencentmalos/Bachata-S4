...
- Read [the 2026-09-08 publication follow-up](docs/validation/v0/followup-publication-2026-09-08.md) before continuing implementation, then [the review it answers](docs/validation/v0/review-publication-2026-09-08.md). The public `GuestAddressSpace` publication path now reaches FEX's translation caches through a `CodeInvalidationSink`, closing P1-C: the same probe on Swan went from executing the stale fixture (rax=17) to the new one (rax=34). M13 now has the two-host-thread pin/unmap race its criterion requires, and M07 additionally requires publication evidence that touches no `CpuContext` method. Acceptance reads 21 PASS / 0 FAIL / 37 NOT_RUN of 58 in-scope cases, 2 deferred by scope. This is still `V0_IN_PROGRESS`: stopping and resuming *running* threads, real typed HLE span lease/callback, and app integration have not been started. Harness assertion counts are not full V0 acceptance counts.
