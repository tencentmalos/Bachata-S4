...
- Read [the 2026-09-08 poison follow-up](docs/validation/v0/followup-poison-2026-09-08.md) before continuing implementation, then [the review it answers](docs/validation/v0/review-poison-2026-09-08.md). A publication transaction now excludes execution for its whole duration: an `ExecutionLease` gates Run/CreateThread, a failed invalidation blocks both execution and re-granting execute until the range is repaired, and unregistering a `CodeInvalidationSink` waits for in-flight callbacks. Device contract 29/29 and guest execution 41/41 on Swan; acceptance 21 PASS / 0 FAIL / 37 NOT_RUN of 58 in-scope cases, 2 deferred. The runner now fails a suite that dies with no output, treats SKIP as not-run, and distinguishes "never started" from "started and crashed". Build entry is `cmake/fex` with `-DV0_ENABLE_FEX=ON -DFEX_BUILD_DIR=<fexcore build>`; the separate `tests/guest_cpu/CMakeLists.txt` added earlier was a duplicate and is gone. Still `V0_IN_PROGRESS`: interrupting *running* threads, real typed HLE, and app integration have not been started. Harness assertion counts are not full V0 acceptance counts.
...
4. The reference ARM64 emulator build is Linux/glibc. The main repository now has independent NDK/bionic FEX execution harnesses; an integrated app backend, JNI entry points, complete allocator/TLS/signal integration and dependency closure remain unvalidated.
